...
internal class IntcodeComputerTest {
    ...
    fun `calculateOpcode1 in position 0`() {
        val program = mutableListOf(1, 5, 6, 3, 99, 30, 40, 50)
        IntcodeComputer(program).calculateIntcode()

        assertEquals(listOf(1, 5, 6, 70, 99, 30, 40, 50), program)
    }

    @Test
    fun `calculateOpcode2 in position 0`() {
        val program = mutableListOf(2, 5, 7, 0, 99, 70, 40, 50)
        IntcodeComputer(program).calculateIntcode()

        assertEquals(listOf(3500, 5, 7, 0, 99, 70, 40, 50), program)
    }
    ...
    fun `calculateOpcode2 in position 1`() {
        ...
        assertEquals(listOf(1002, 4, 3, 4, 99), program)
    }
    ...
    fun `calculateIntcode results in the correct memory state`(inputStr: String, expectedStr: String) {
        val program = inputStr.split(",").map { it.toInt() }.toMutableList()
    ...
    @CsvSource("1, 999",
        "8, 1000",
        "9, 1001")
    fun `test opcodes 1 to 8 are working`(input: Int, expected: Int) {
